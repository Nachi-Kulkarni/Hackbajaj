...
3. Provide responses in structured JSON format with ALL required fields
...
You MUST respond with valid JSON containing ALL of these fields based on query type:

**For AGE-BASED queries, include:**
{
  "answer": "Direct answer to the user's question",
  "confidence_score": 0.95,
  "age_restrictions": "Any age-related restrictions or null if none",
  "minimum_age": "Minimum age requirement or null if not specified",
  "maximum_age": "Maximum age limit or null if not specified",
  "source_sections": ["List of document sections where information was found"],
  "key_details": {},
  "limitations": "Any limitations or missing information"
}

**For PROCEDURE-BASED queries, include:**
{
  "answer": "Direct answer to the user's question",
  "confidence_score": 0.95,
  "procedure_steps": ["Step 1", "Step 2", "Step 3"] or null if not available,
  "required_documents": ["Document 1", "Document 2"] or null if not specified,
  "timeline": "Expected timeline or null if not specified",
  "source_sections": ["List of document sections where information was found"],
  "key_details": {},
  "limitations": "Any limitations or missing information"
}

**For LOCATION-BASED queries, include:**
{
  "answer": "Direct answer to the user's question",
  "confidence_score": 0.95,
  "covered_locations": ["Location 1", "Location 2"] or null if not specified,
  "geographical_limits": "Geographic limitations or null if none",
  "exclusions": ["Excluded area 1", "Excluded area 2"] or null if none,
  "source_sections": ["List of document sections where information was found"],
  "key_details": {},
  "limitations": "Any limitations or missing information"
}

**For POLICY_DURATION queries, include:**
{
  "answer": "Direct answer to the user's question",
  "confidence_score": 0.95,
  "policy_term": "Policy duration or null if not specified",
  "renewal_options": "Renewal information or null if not available",
  "effective_dates": "Policy effective dates or null if not specified",
  "source_sections": ["List of document sections where information was found"],
  "key_details": {},
  "limitations": "Any limitations or missing information"
}

**For COMPLEX_MULTI_CRITERIA queries, include:**
{
  "answer": "Direct answer to the user's question",
  "confidence_score": 0.95,
  "criteria_analysis": "Analysis of multiple criteria or null if not available",
  "interactions": "How different elements interact or null if not clear",
  "recommendations": "Recommendations based on analysis or null if none",
  "source_sections": ["List of document sections where information was found"],
  "key_details": {},
  "limitations": "Any limitations or missing information"
}
...
**IMPORTANT RULES:**
1. ALWAYS include ALL required fields for the query type, even if the value is null
2. Use null (not "null" string) for missing information
3. Ensure JSON is properly formatted and valid
4. Never omit required fields - this causes validation failures
5. Be conservative with confidence scores when information is limited
"""
...
**Instructions:**
1. Analyze the provided insurance document carefully
2. Identify the query type from the categories above
3. Extract relevant information based on the query type
4. Provide your response in the EXACT JSON format specified for that query type
5. Include ALL required fields for the identified query type, even if the value is null
6. Focus only on information that can be found or reasonably derived from the document content
7. If information is not available, use null (not string "null") for missing fields
8. Set confidence score based on the availability and clarity of information in the document

**CRITICAL:** You MUST include all required fields for the query type. Missing fields will cause validation failures.
"""
...
"""
    ...
    def parse_llm_response(self, response: str) -> Dict[str, Any]:
        ...
        logger.error(f"Failed to parse JSON response. Response preview: {response[:200]}...")
        return None
    ...
    def validate_response_structure(self, parsed_json: Dict, query_type: QueryType) -> bool:
        ...
        # Check query-specific fields with more lenient validation
        ...
        if template:
            ...
            if len(missing_fields) == len([f for f in template.expected_fields if f not in required_fields]):
                logger.warning(f"All query-specific fields missing for {query_type.value}")
                return False
    ...
    def test_prompt_with_query(self, document_text: str, query: str, query_type: QueryType, 
        ...
        # Updated success criteria - more lenient but still maintains quality
        success = (structure_valid and 
                  quality_score > 0.5 and 
                  confidence_score is not None and 
                  confidence_score >= 0.0)  # Allow any valid confidence score
        ...
        if not structure_valid:
            ...
            error = "Missing confidence score"
